...
class TmxTileSet
{
	...
	public function new(data:Dynamic)
	{
		...
		{
			//trace(Assets.getText(Std.string("maps/" + source.att.source)));
			source = new Fast(Xml.parse(Assets.getText(Std.string("maps/" + source.att.source))));
			//xml.elementsNamed("tileset").next().set("firstgid", Std.string(firstGID));
			//source = new Fast(xml).node.tileset;
			source = source.node.tileset;
			//trace(source.att.resolve("name"));
			trace(source.node.image.att.source);
